...
select 
    ename,
    sal,
    nvl(decode(job,'ANALYST',sal*1.10,
                           'CLERK',sal*1.15,
                           'MANAGER',sal*1.20),0) 인상급여 
from emp;
...
select 
    count(decode(to_char(hiredate,'mm'),01,1))||'명' "1월입사인원",
    count(decode(to_char(hiredate,'mm'),02,1))||'명' "2월입사인원",
    count(decode(to_char(hiredate,'mm'),03,1))||'명' "3월입사인원",
    count(decode(to_char(hiredate,'mm'),04,1))||'명' "4월입사인원",
    count(decode(to_char(hiredate,'mm'),05,1))||'명' "5월입사인원",
    count(decode(to_char(hiredate,'mm'),06,1))||'명' "6월입사인원"
from emp 
where to_char(hiredate,'mm')<=6;
